...
import { ArrowRight, DraftingCompass, Flower2, Instagram, Leaf, Mail, MapPin, MessageCircle, Phone, Ruler, Sprout, Trees, Youtube } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Jasa desain, pembangunan, dan perawatan taman profesional untuk hunian dan ruang komersial." },
      { property: "og:title", content: "Aksara Landscape" },
      { property: "og:description", content: "Mewujudkan ruang hijau yang indah, fungsional, dan berkelanjutan." },
...
const projects = [
  [modernImage, "Taman Rumah Modern"], [rooftopImage, "Rooftop Garden Office"],
  [verticalImage, "Taman Vertikal Lobby"], [villaImage, "Taman Villa Tropis"], [minimalImage, "Taman Minimalis"],
];

const steps = [
  { icon: MessageCircle, number: "01", title: "Konsultasi", text: "Diskusi kebutuhan dan survei lokasi." },
  ...
  { icon: Sprout, number: "04", title: "Perawatan", text: "Perawatan berkala untuk hasil optimal." },
];
    ...
    <main>
      <header className="site-header">
        ...
        <nav aria-label="Navigasi utama">
          <a href="#beranda">Beranda</a><a href="#tentang">Tentang Kami</a><a href="#layanan">Layanan</a><a href="#proyek">Proyek</a><a href="#proses">Proses</a><a href="#artikel">Artikel</a><a href="#kontak">Kontak</a>
        </nav>
        <a className="button button-green header-cta" href="#kontak">Konsultasi Gratis</a>
      </header>

      <section id="beranda" className="hero">
        <img src={heroImage} alt="Taman tropis modern dengan pencahayaan hangat" width={1600} height={900} />
        <div className="hero-shade" />
        <div className="hero-inner wrap">
          <div className="hero-copy">
            <h1>Mewujudkan<br />Ruang Hijau,<br />Menghidupkan<br />Kehidupan</h1>
            <p>AKSARA LANDSCAPE menghadirkan solusi landscape profesional yang menggabungkan keindahan alam, fungsi, dan keberlanjutan.</p>
            <div className="hero-actions"><a className="button button-green" href="#proyek">Lihat Proyek Kami</a><a className="button button-light" href="#kontak">Konsultasi Gratis</a></div>
          </div>
          <div className="hero-panel">
            <Feature icon={Leaf} title="Desain Kreatif" text="Desain lanskap yang estetis, fungsional, dan selaras dengan lingkungan." />
            <Feature icon={Flower2} title="Kualitas Terbaik" text="Material dan tanaman pilihan dengan standar kualitas tinggi." />
            <Feature icon={Sprout} title="Perawatan Profesional" text="Perawatan rutin untuk menjaga keindahan dan kesehatan taman Anda." />
          </div>
        </div>
      </section>

      <section id="tentang" className="about wrap section-pad">
        <div className="about-copy"><span className="eyebrow">TENTANG KAMI</span><h2>Menyatukan Alam,<br />Desain, dan Kehidupan</h2><p>AKSARA LANDSCAPE adalah perusahaan yang bergerak di bidang jasa desain, pembangunan, dan perawatan lanskap. Kami percaya bahwa ruang hijau yang dirancang dengan baik dapat meningkatkan kualitas hidup dan menciptakan keseimbangan antara manusia dan alam.</p><a className="button button-green" href="#layanan">Selengkapnya Tentang Kami</a></div>
      ...
      <section id="proses" className="process"><div className="wrap process-inner"><div className="process-heading"><h2>Proses Kerja Kami</h2><p>Kami menerapkan proses kerja yang terstruktur untuk memastikan hasil terbaik di setiap proyek.</p></div><div className="steps">{steps.map(({ icon: StepIcon, number, title, text }, i) => <div className="step" key={number}><span className="step-icon"><StepIcon /></span><strong>{number}<br />{title}</strong><p>{text}</p>{i < 3 && <i />}</div>)}</div></div></section>
...
function Feature({ icon: Icon, title, text }: { icon: typeof Leaf; title: string; text: string }) {
  return <div className="feature"><Icon /><div><h3>{title}</h3><p>{text}</p></div></div>;
}